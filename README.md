...
`policy_manage`, `group_role_manage`, `terminal_control`, `session_events_ws`,
`rendered_events_ws`, `terminal_ws`, and `bot_gateway_ws`; omitting it grants all
current scopes. Managed device credentials need `device_manage` to call
...
`/api/v1/chat-contexts/{id}/roles*`, and `terminal_control` to call terminal start,
restart, input, or lifecycle run-once HTTP routes.
