...
handshake JSON with schema snapshots, Claude Hook and Codex provider-captured schema
projections, API-token or device-key headers, event submission, response polling,
blocking waits for adapter-originated interactions, explicit offline outbox flushing, and
local schema/provider-version matrix inspection:
...
artifact paths inside the same section or malformed sha256 digests are malformed
evidence. The
...
release review while rejecting unindexed ZIP entries, checking indexed artifact section
ownership, requiring unique indexed artifact paths and archive paths, and recomputing
and checking artifact/checklist summary counts. Set
