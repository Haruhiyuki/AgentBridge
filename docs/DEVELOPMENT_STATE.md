...
- Filtered audit API through `GET /api/v1/audit`, backed by repository-level newest-first queries bounded by `limit` and supporting action, actor, trace, project, session, and interaction filters for operational review; the same filters can be exported through `GET /api/v1/audit/export` as JSON, CSV, an HMAC-SHA256 signed archive when `AGENTBRIDGE_AUDIT_ARCHIVE_SIGNING_KEY(_FILE)` is configured, or an Ed25519/RSA-PSS/ECDSA signed archive when `AGENTBRIDGE_AUDIT_ARCHIVE_SIGNING_PRIVATE_KEY_FILE` is configured, and SQLAlchemy uses indexed action/actor columns before payload-level filters.
...
- Terminal Lifecycle Admin Web page shows tracked sessions, exit/loss counts, auto-restart attempts, command allowlist patterns, policy blocks, backend supervision state, and a bounded lifecycle run-once action.
...
- Managed device identities now require `session_event_ingest` for Terminal Agent event ingestion through `POST /api/v1/sessions/{id}/events`.
...
1. Harden PTY host recovery beyond watchdog plus command restart, including cross-platform socket/pipe cleanup and Windows ConPTY/Named Pipe parity.
2. Expand the Admin Web UI beyond system health, project/session operations, queue controls, interaction/approval operations, audit/event live/search exploration, access policy, terminal lifecycle, device identity management, and Bot delivery operations, including additional live dashboards and normalized payload search.
3. Extend the HTTP API/WebSocket/admin/device-key/client-cert-fingerprint gates with KMS-backed or offline remote CA signing, native production TLS/key lifecycle enforcement, and deeper command/payload-aware or method-specific resource/action device authorization.
