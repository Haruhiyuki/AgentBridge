...
- Project create APIs, the Project/Session Admin UI, and `/agent project create` expose `max_active_sessions` and `max_queued_turns`; Session and Turn creation reject over-quota projects with `QUOTA_EXCEEDED` until inactive sessions or non-queued Turns release capacity.
