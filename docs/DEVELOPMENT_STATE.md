...
- The Admin Web UI is intentionally API-backed and build-free for now. It has a small `/admin` entrypoint, project/session operations for project inventory, workspace registration, session creation, and session closure, interaction/approval operations for listing, filtering, creating, answering, voting, and cancelling interactions, access policy editing, terminal lifecycle inspection/run-once, and Bot delivery operations for records, retry worker status, due retry, and rate limits. Browser authentication hardening and richer live dashboards remain future work.
...
2. Expand the Admin Web UI beyond project/session operations, interaction/approval operations, access policy, terminal lifecycle, and Bot delivery operations, including hardened browser authentication, live event dashboards, and richer audit/event exploration.
