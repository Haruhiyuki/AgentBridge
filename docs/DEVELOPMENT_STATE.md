...
- Managed device identity scopes are mostly coarse transport gates (`http_api`, event WebSockets, terminal WebSocket, and Bot Gateway WebSocket), not a replacement for actor RBAC, access-policy rules, or future resource-level authorization. `bot_gateway_manage`, `command_execute`, `device_manage`, `policy_manage`, `group_role_manage`, `chat_context_manage`, `project_manage`, `session_manage`, `session_send`, `session_event_ingest`, `interaction_manage`, and `terminal_control` are the first action-scoped managed-device permissions; they are required for managed credentials to call Bot Gateway HTTP mutation APIs, command execution, device identity APIs, access/approval policy APIs, chat-context role APIs, chat-context creation/pointer write APIs, project/workspace/binding write APIs, session lifecycle and writer-lease write APIs, direct session turn enqueue APIs, Terminal Agent session event ingestion APIs, interaction/approval write APIs, and terminal-control HTTP APIs. Command execution remains a broad entrypoint whose effects are still constrained by the command actor's RBAC and access-policy decision; direct turn enqueue likewise still checks the turn actor's `session.send` permission. Managed device identities can bind device keys and client certificate fingerprints to the same scope and revocation lifecycle. The certificate fingerprint gates assume a trusted TLS-terminating proxy has already verified mTLS and stripped spoofable inbound fingerprint headers before setting the configured header for AgentBridge.
