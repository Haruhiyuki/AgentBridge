...
- Production API/WebSocket hardening with mTLS, device certificates, finer-grained resource/action-scoped device authorization, and certificate/device rotation workflows beyond the current token-file/device-key and transport-scope gates.
...
- `AGENTBRIDGE_API_TOKEN(_FILE)`, `AGENTBRIDGE_WS_TOKEN(_FILE)`, `AGENTBRIDGE_ADMIN_TOKEN(_FILE)`, `AGENTBRIDGE_DEVICE_KEYS`, and persisted device identities are the current MVP gates for HTTP API clients, WebSocket clients, built-in admin pages, and per-device client keys. Token files are reread per request/connection and never disable a configured gate when unreadable or empty; if no other token source exists, token auth fails closed. Browser Admin sessions use an HttpOnly cookie for REST calls and same-origin event WebSocket streams so the unlock token does not need to be readable from JavaScript. Managed device keys are stored as salted PBKDF2 hashes and fail closed once any managed identity exists, even if all managed identities are revoked. These gates are intentionally simpler than the design's production mTLS/certificate model, which remains future hardening work.
...
3. Replace the MVP HTTP API/WebSocket/admin/device-key gates with mTLS, device certificates, finer-grained resource/action-scoped device authorization, and certificate/device rotation workflows.
