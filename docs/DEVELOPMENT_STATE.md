...
- The Project/Session Admin UI shows active Turn, queued-Turn count/pause state, and current writer-lease ownership in the Session table, plus the selected Session queue with `queue_version`, pause state, queued Turn rows, and controls for refresh, pause, resume, and confirmed clear.
