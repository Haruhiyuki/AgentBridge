...
- Device Identity Admin Web page lists active/revoked managed devices with allowed scopes, allowed resource IDs, certificate fingerprint count, and last-used timestamps, creates or rotates device keys, edits resource allowlists and managed certificate fingerprints, runs add/remove certificate fingerprint rotation, displays the generated key once, and revokes selected device identities through the managed device API.
...
- Production API/WebSocket hardening with CA-backed certificate issuance/renewal workflows, native TLS termination guidance, and deeper command/payload-aware or method-specific resource/action device authorization beyond the current token-file/device-key/client-cert-fingerprint, transport-scope, managed fingerprint rotation, and visible resource-ID gates.
...
3. Extend the HTTP API/WebSocket/admin/device-key/client-cert-fingerprint gates with CA-backed certificate issuance/renewal workflows and deeper command/payload-aware or method-specific resource/action device authorization.
