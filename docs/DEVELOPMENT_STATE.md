...
- Turn queue management APIs and commands now cover `queue list`, `queue remove`, `queue clear`, and `queue move`; maintainers can reorder queued Turns through optimistic `queue_version` checks, operators may remove their own queued Turns, and clearing or removing another actor's queued Turn requires session management permission.
...
- Managed device identities now require `session_manage` for session REST writes: create/close sessions, acquire/release leases, clear queues, and reorder queued Turns.
...
- Remaining queue operations: pause, resume, and explicit clear confirmation UX.
...
- Queue ordering uses a dedicated `queue_order` field rather than mutating `queued_at`, so reorder operations do not corrupt daily Turn quota accounting. Queue list responses return an opaque `queue_version`; reorder requires it, and remove/clear accept it for compare-and-swap protection.
