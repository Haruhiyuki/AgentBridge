...
- Remaining queue operation UX: explicit clear confirmation.
...
- Queue ordering uses a dedicated `queue_order` field rather than mutating `queued_at`, so reorder operations do not corrupt daily Turn quota accounting. Queue list responses return an opaque `queue_version` and `queue_paused`; reorder and pause/resume require the version, remove/clear accept it for compare-and-swap protection, and paused queues reject new `turn.started` lifecycle events until resumed.
