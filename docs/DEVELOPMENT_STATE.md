...
- Cross-stream rendered event search through `GET /api/v1/events/rendered`, using the same operational filters and returning render documents plus OneBot/plain-text messages for operator-facing notifications.
...
3. Extend the HTTP API/WebSocket/admin/device-key/client-cert-fingerprint gates with certificate renewal automation, platform-specific certificate scan delivery fan-out, production CA key custody/TLS guidance, and deeper command/payload-aware or method-specific resource/action device authorization.
