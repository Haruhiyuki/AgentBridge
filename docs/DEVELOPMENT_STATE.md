...
- Built-in admin entrypoint at `/admin`, access policy editor at `/admin/access-policy`, terminal lifecycle dashboard at `/admin/terminal-lifecycle`, and Bot delivery operations dashboard at `/admin/bot-delivery`; focused route coverage verifies the admin pages link to the underlying operational APIs.
...
- Broader Admin Web UI beyond access policy, terminal lifecycle, and Bot delivery operations.
...
- The Admin Web UI is intentionally API-backed and build-free for now. It has a small `/admin` entrypoint, access policy editing, terminal lifecycle inspection/run-once, and Bot delivery operations for records, retry worker status, due retry, and rate limits. Broader project/session administration, authentication hardening, and richer live dashboards remain future work.
...
2. Expand the Admin Web UI beyond access policy, terminal lifecycle, and Bot delivery operations, including project/session management and hardened browser authentication.
