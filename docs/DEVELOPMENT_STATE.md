...
- Local Terminal Agent daemon using JSONL over a Unix socket with token authentication, chmod `0600` socket permissions, and default same-OS-user peer UID verification where Unix peer credentials are available.
...
- The local Terminal Agent socket is token-gated, chmodded to `0600`, and checks same-user Unix peer credentials by default. Production hardening still needs token rotation and Windows named-pipe parity.
