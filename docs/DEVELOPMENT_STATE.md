...
- `pty_host` client backend can auto-start `agentbridge-pty-host` with `AGENTBRIDGE_TERMINAL_PTY_HOST_AUTO_START=true`; it removes unreachable stale Unix socket files, waits for host health, and retries the original request once.
- `pty_host` supervisor treats `PERMISSION_DENIED` health probes as a live-host token mismatch, preserves the active socket, records `last_error`, and fails closed instead of starting a competing host process.
