...
- Database-managed device identities through `/api/v1/device-identities`, storing salted PBKDF2 key hashes and managed client certificate fingerprints, returning generated keys only when creating key-backed identities, supporting certificate-only identities without an implicit fallback key, supporting revoke, authorizing REST/WebSocket clients through device keys or proxy-verified certificate fingerprints, enforcing coarse transport scopes plus `command_execute` for command execution, `device_manage` for device-identity administration, `policy_manage` for access/approval policy administration, `group_role_manage` for chat-context role administration, `project_manage` for project/workspace/binding writes, `session_manage` for session lifecycle and writer-lease writes, and `terminal_control` for terminal-control HTTP operations, and tracking `last_used_at` on successful managed-device authentication.
