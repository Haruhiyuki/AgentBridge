...
- Project create APIs, the Project/Session Admin UI, and `/agent project create` expose `max_active_sessions`, `max_running_turns`, and `max_queued_turns`; Session and Turn lifecycle operations reject over-quota projects with `QUOTA_EXCEEDED` until inactive sessions or non-running/non-queued Turns release capacity.
