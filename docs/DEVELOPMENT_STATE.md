...
- System Health Admin Web page gives a read-only operational summary of `/api/v1/health`, terminal lifecycle monitor status, Bot retry worker status, Bot rate-limit policies, and managed-device endpoint reachability.
...
3. Extend the HTTP API/WebSocket/admin/device-key/client-cert-fingerprint gates with certificate renewal automation, scheduled certificate health scan execution, production CA key custody/TLS guidance, and deeper command/payload-aware or method-specific resource/action device authorization.
