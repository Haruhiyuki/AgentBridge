...
- Production-grade PTY host supervision: automatic host process spawning, health checks, crash restart policy, stale socket cleanup across platforms, and Windows ConPTY/Named Pipe parity.
...
- `agentbridge-pty-host` is the first independent PTY host process. It keeps PTY sessions alive across API/daemon client restarts when the host process itself remains running. It is still manually started and lacks a supervisor that can spawn/restart the host process, detect stale sockets, or preserve PTYs across host crashes.
...
1. Add production PTY host supervision around `agentbridge-pty-host`, including host auto-spawn/restart, stale socket cleanup, health checks, and Windows ConPTY/Named Pipe parity.
