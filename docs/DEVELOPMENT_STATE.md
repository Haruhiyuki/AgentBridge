...
- Built-in admin entrypoint at `/admin`, access policy editor at `/admin/access-policy`, and terminal lifecycle dashboard at `/admin/terminal-lifecycle`; focused route coverage verifies the admin pages link to the underlying operational APIs.
...
- Broader Admin Web UI beyond access policy and terminal lifecycle operations.
...
- The Admin Web UI is intentionally API-backed and build-free for now. It has a small `/admin` entrypoint, access policy editing, and terminal lifecycle inspection/run-once; broader project/session administration, authentication hardening, and richer live dashboards remain future work.
...
2. Expand the Admin Web UI beyond access policy and terminal lifecycle operations, including project/session management, Bot delivery operations, and hardened browser authentication.
