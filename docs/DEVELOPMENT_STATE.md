...
- Production API/WebSocket hardening with certificate renewal scheduling, native TLS termination guidance, CA key custody/runbook hardening beyond configured PEM CA files, and deeper command/payload-aware or method-specific resource/action device authorization beyond the current token-file/device-key/client-cert-fingerprint, transport-scope, CA-backed CSR issuance/renewal, managed fingerprint rotation, certificate metadata inventory, audited certificate health scans, background certificate scan worker, operator-readable scan event rendering, filtered/configured Bot Gateway scan fan-out, certificate expiry health summaries, and visible resource-ID gates.
...
3. Extend the HTTP API/WebSocket/admin/device-key/client-cert-fingerprint gates with certificate renewal scheduling, production CA key custody/TLS guidance, and deeper command/payload-aware or method-specific resource/action device authorization.
