...
- Managed device identity scopes are mostly coarse transport gates (`http_api`, event WebSockets, terminal WebSocket, and Bot Gateway WebSocket), not a replacement for actor RBAC, access-policy rules, or future resource-level authorization. `device_manage`, `policy_manage`, and `group_role_manage` are the first action-scoped managed-device permissions; they are required for managed credentials to call device identity APIs, access/approval policy APIs, and chat-context role APIs. Managed device identities can bind device keys and client certificate fingerprints to the same scope and revocation lifecycle. The certificate fingerprint gates assume a trusted TLS-terminating proxy has already verified mTLS and stripped spoofable inbound fingerprint headers before setting the configured header for AgentBridge.
