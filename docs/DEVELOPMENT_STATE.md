...
- Database-managed device identities through `/api/v1/device-identities`, storing salted PBKDF2 key hashes and managed client certificate fingerprints, returning generated keys only when creating key-backed identities, supporting certificate-only identities without an implicit fallback key, supporting revoke, authorizing REST/WebSocket clients through device keys or proxy-verified certificate fingerprints, enforcing coarse transport scopes plus `audit_read` for audit/event history HTTP reads, `bot_gateway_read` for Bot Gateway HTTP reads, `bot_gateway_manage` for Bot Gateway HTTP mutations, `onebot_event_ingest` for OneBot inbound events, `command_parse` for command parsing, `command_execute` for command execution, `device_manage` for device-identity administration, `policy_manage` for access/approval policy administration, `group_role_manage` for chat-context role administration, `chat_context_manage` for chat-context creation/pointer writes, `project_manage` for project/workspace/binding writes, `session_manage` for session lifecycle and writer-lease writes, `session_send` for direct session turn enqueue, `session_event_ingest` for Terminal Agent session event ingestion, `interaction_manage` for interaction/approval writes, `terminal_read` for terminal-read HTTP operations, and `terminal_control` for terminal-control HTTP operations, and tracking `last_used_at` on successful managed-device authentication.
