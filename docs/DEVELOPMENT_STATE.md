...
- The Project/Session Admin UI shows the selected Session queue with `queue_version`, pause state, queued Turn rows, and controls for refresh, pause, resume, and confirmed clear.
...
- Managed device identities now require `session_read` for session REST reads: list/show sessions and queued Turns.
