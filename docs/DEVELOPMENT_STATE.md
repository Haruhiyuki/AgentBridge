...
- Bot Gateway HTTP delivery can target session streams or filtered cross-stream semantic events. Cross-stream delivery requires at least one event filter so an operator cannot accidentally fan out unrelated recent events to a chat context.
...
3. Extend the HTTP API/WebSocket/admin/device-key/client-cert-fingerprint gates with certificate renewal automation, automatic scan notification routing policies, production CA key custody/TLS guidance, and deeper command/payload-aware or method-specific resource/action device authorization.
