...
- Database-managed device identities through `/api/v1/device-identities`, storing salted PBKDF2 key hashes and managed client certificate fingerprints, returning generated keys only when creating key-backed identities, supporting certificate-only identities without an implicit fallback key, supporting revoke, authorizing REST/WebSocket clients through device keys or proxy-verified certificate fingerprints, enforcing coarse transport scopes plus `device_manage` for device-identity administration, `policy_manage` for access/approval policy administration, `group_role_manage` for chat-context role administration, `project_manage` for project/workspace/binding writes, `session_manage` for session lifecycle and writer-lease writes, and `terminal_control` for terminal-control HTTP operations, and tracking `last_used_at` on successful managed-device authentication.
...
- Managed device identity scopes are mostly coarse transport gates (`http_api`, event WebSockets, terminal WebSocket, and Bot Gateway WebSocket), not a replacement for actor RBAC, access-policy rules, or future resource-level authorization. `device_manage`, `policy_manage`, `group_role_manage`, `project_manage`, `session_manage`, and `terminal_control` are the first action-scoped managed-device permissions; they are required for managed credentials to call device identity APIs, access/approval policy APIs, chat-context role APIs, project/workspace/binding write APIs, session lifecycle and writer-lease write APIs, and terminal-control HTTP APIs. Managed device identities can bind device keys and client certificate fingerprints to the same scope and revocation lifecycle. The certificate fingerprint gates assume a trusted TLS-terminating proxy has already verified mTLS and stripped spoofable inbound fingerprint headers before setting the configured header for AgentBridge.
