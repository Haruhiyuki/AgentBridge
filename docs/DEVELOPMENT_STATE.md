...
- Turn queue management APIs and commands now cover `queue list`, `queue remove`, and `queue clear`; operators may remove their own queued Turns, while clearing or removing another actor's queued Turn requires session management permission.
...
- Managed device identities now require `session_read` for session REST reads: list/show sessions.
...
- Remaining queue operations: reorder/move, pause, resume, optimistic queue versioning, and explicit clear confirmation UX.
