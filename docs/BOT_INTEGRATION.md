...
- 服务端以 `f"{platform}:{user_id}"` 作为 actor 身份，按请求里的 `default_roles` 鉴权。
- **身份 → 角色** 的映射当前由适配器决定（bot 把用户角色放进 `default_roles`）。这样平台特有的
  账号体系不必抽象进服务端。角色对应的**权限**（能做什么）则完全由服务端的 RBAC 拥有，可经
  `GET /api/v1/roles`、`PUT /api/v1/roles/{name}` 等管理。
