...
## 机器可读契约（OpenAPI / Swagger）

- `GET /openapi.json`、Swagger UI `GET /docs`（均走同一 API 令牌鉴权）。
- 本指南的核心端点在 OpenAPI 里归到 **`bot-integration`** 标签，便于一眼找到三步契约。

## 参考与对照

- 最小参考（推荐起点）：[`examples/minimal_bot.py`](../examples/minimal_bot.py)，
  Python 客户端 [`agentbridge.bot_client`](../src/agentbridge/bot_client.py)。
- NoneBot 适配器（**进阶**，含原生命令注册、消息编辑等富交互）：`src/agentbridge/nonebot_plugin.py`。
